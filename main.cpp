...
float GetDistance(XMFLOAT3 pos1, XMFLOAT3 pos2) {
	int distance = powf(pos1.x - pos2.x, 2) + powf(pos1.y - pos2.y, 2);
	return distance;
}

XMFLOAT3 QuadraticBenzierCurve(XMFLOAT3 startpos, XMFLOAT3 arcpos, XMFLOAT3 endpos, float percentage) {
	return XMFLOAT3((powf(1 - percentage, 2) * startpos.x)
		+ (2 * (1 - percentage) * percentage * arcpos.x)
		+ (powf(percentage, 2) * endpos.x),
		(powf(1 - percentage, 2) * startpos.y)
		+ (2 * (1 - percentage) * percentage * arcpos.y)
		+ (powf(percentage, 2) * endpos.y),
		startpos.z);
}