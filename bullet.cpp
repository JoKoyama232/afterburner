...
void UpdateBullet(void)
{
	...
	for (int i = 0; i < BULLET_MAX; i++)
	{
		...
		{								// Yes
			ENEMY g_Enemy = GetEnemy()[g_Bullet[i].target_ID];
			// アニメーション  
			...
			// バレットの移動処理
			//XMVECTOR pos  = QuadraticBenzierCurve();
			//XMVECTOR move = XMLoadFloat3(&g_Bullet[i].move);
			//pos += move;
			g_Bullet[i].pos = QuadraticBenzierCurve(g_Bullet[i].pos, g_Bullet[i].pos_arc, g_Enemy.pos, g_Bullet[i].progress);
			g_Bullet[i].progress += 0.05f;
			if (g_Bullet[i].progress > 1.0f) {
				g_Bullet[i].use = false;
				g_Bullet[i].progress = 0;
			}
...
void SetBullet(XMFLOAT3 pos)
{
	...
	for (int i = 0; i < BULLET_MAX; i++)
	{
		if (g_Bullet[i].use == FALSE)		// 未使用状態のバレットを見つける
		{
			...
			g_Bullet[i].pos_prev = g_Bullet[i].pos;			// 座標をセット
			int enemyID = 0;
			for (int e = 1; e < ENEMY_MAX; e++) {
				if (GetDistance(g_Bullet[i].pos, g_Enemy[enemyID].pos) > GetDistance(g_Bullet[i].pos, g_Enemy[e].pos)) {
					enemyID = e;
				}
			}
			g_Bullet[i].target_ID = enemyID;
			g_Bullet[i].pos_arc = g_Bullet[i].pos;
			g_Bullet[i].pos_arc.x += 50.0f;
